...
###  Input the enemy count at the beginning of console
...
def show_level(level):
    levelpass = font.render("Level"+level, True, (255, 0, 0))
    screen.blit(levelpass,(10, 35))
...
while running:
    ...
    for i in range(num_of_enemies):
        ...
        if collision:
            ...
            bullet_state = "ready"
            if enemyY[i] < 440:
                score_value += round(num_of_enemies*2/3*level)
            enemyX[i] = random.randint(0, 735)
            enemyY[i] = random.randint(50, 150)
    ...
    if score_value > 5 and level == 1:
        level2()


    player(playerX, playerY)
    show_score(textX, textY)
    show_level(str(level))
